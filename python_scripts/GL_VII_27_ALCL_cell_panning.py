...
file_name = 'GL_VII_27_ALCL_cell_panning_master_list.csv'
file_path = os.path.join(os.getcwd(), file_name)
dictionary = 'GE.xlsx'
number_sdbs = pd.read_excel(os.path.join(os.getcwd(), dictionary)).shape[0]
replicates = 3
targets = 4
figure = 'Fig4b_SFig6c.xlsx'
...
input, s_rr, s_ru, u_rr, u_ru = data[:, 16].reshape(number_sdbs, 1), \
                                data[:, 17:20], \
                                data[:, 20:23], \
                                data[:, 23:26], \
                                data[:, 26:29]

fc = np.concatenate(((s_rr/input).reshape(number_sdbs*replicates, 1),
                     (s_ru/input).reshape(number_sdbs*replicates, 1),
                     (u_rr/input).reshape(number_sdbs*replicates, 1),
                     (u_ru/input).reshape(number_sdbs*replicates, 1),
                     input.reshape(number_sdbs,1)))

cells = np.array(['s_rr']*number_sdbs*replicates + ['s_ru']*number_sdbs*replicates
                 + ['u_rr']*number_sdbs*replicates + ['u_ru']*number_sdbs*replicates
                 + ['Input']*number_sdbs
