...
file_name = 'GL_VII_63 tcell vs bcells_master_list.csv'
...
if not os.path.exists(file_path):
    ...
    for file in files:
        ...
        for reads in primers:
            if file == '20230712-1618LILAAooBO-GL.txt' and reads not in ['R9F1_RN1RP1', 'R9F2_RN1RP2', 'R9F3_RN1RP3']:
                pass
            else:
...
input, tcell, bcell = np.mean(data[:,18:21 ], axis=1).reshape(number_sdbs,1), \
                                data[:, 12:15], \
                                data[:, 15:18]
...
cells = np.array(['T-cell'] * number_sdbs*replicates + ['B-cell'] * number_sdbs*replicates
...
def mbp_norm(df_cell):
    ...
    mbp_values = df2_cell.loc[df_cell['lectin'].isin(['MBP'])]
    interval = int(mbp_values.shape[0] / targets)
    means = [mbp_values[0:interval]['fold'].mean(),
             mbp_values[interval:interval * 2]['fold'].mean()]
...
plot = sns.catplot(x='cells', y='fold',
                   hue='lectin',
                   kind='bar', data=test,
                   ...
                   edgecolor='black',
                   ci=95, errcolor='black', capsize=0.01, errwidth=1,
                   legend=False, aspect=10 / 4
                   )
...
labels = [0, 1, 5, 10, 100, 200, 400, 600]
ticks = [x ** (1 / 2) for x in labels]
plt.yticks(ticks, labels)

plt.legend('')
plt.savefig('all_cells.pdf', transparent=True)
plt.show()
