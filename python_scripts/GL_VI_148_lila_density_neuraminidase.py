...
file_name = 'GL_VI_148_lila_density_neuraminidase_master_list.csv'
...
input, wt, wt_n, cmas, cmas_n = data[:, 16].reshape(number_sdbs, 1), \
                                data[:, 17:20], \
                                data[:, 20:23], \
                                data[:, 23:26], \
                                data[:, 26:29]
...
fc = np.concatenate(((wt/input).reshape(number_sdbs*replicates, 1),
                     (wt_n/input).reshape(number_sdbs*replicates, 1),
                     (cmas/input).reshape(number_sdbs*replicates, 1),
                     (cmas_n/input).reshape(number_sdbs*replicates, 1),
                     (input).reshape(number_sdbs,1)))
cells = np.array(['WT']*number_sdbs*replicates + ['WT_n']*number_sdbs*replicates +
                 ['CMAS']*number_sdbs*replicates + ['CMAS_n']*number_sdbs*replicates + ['Input']*number_sdbs).\
...
if __name__ == '__main__':
    ...
    plt.legend('')
    plt.savefig('GL-VI_148-neuraminidase.pdf', transparent=True)
    plt.show()
