...
file_name = 'GL_VI_148_lila_v1_3_master_list.csv'
...
input, wt, cmas = data[:, 10].reshape(number_sdbs, 1), \
                  data[:, 11:14], \
                  data[:, 14:17]
...
cells = np.array(['WT']*number_sdbs*replicates + ['CMAS']*number_sdbs*replicates +
                 ['Input']*number_sdbs).reshape((number_sdbs*replicates*(targets+1)-
                                                 (number_sdbs*2), 1))
...
if __name__ == '__main__':
    ...
    order = ['WT']
    hue_order=['Siglec-7-[20]', 'Siglec-7-[3]', 'Siglec-7-[1]' ]

    plot = sns.barplot(x='cells', y='fold', hue='lectin',
                        order=order,
                       hue_order=hue_order,
                      data=test,
                       palette=['#73a051', '#84aa66', '#b2ca9d',

                                ],
                       edgecolor='black',
                       ci=False, errcolor='black', capsize=0.01, errwidth=1.5
